...
| **Builds on** | [HLD v1.1](../hld/01-ingest.md) · [Patterns](../patterns/01-ingest-patterns.md) · [LLD v1.1](../lld/01-ingest.md) |
| **Region** | `ca-central-1` primary · `ca-west-1` for disaster recovery |
| **IaC** | AWS CDK (Python) |
| **Diagrams** | `diagrams/mermaid/01-ingest/01j` → `01m` (render-checked) |
...
```
*Source: `diagrams/mermaid/01-ingest/01j-aws-architecture.mmd`*
...
```
*Source: `diagrams/mermaid/01-ingest/01k-step-functions.mmd`*
...
flowchart LR
  ...
  class S3G,IFE ep
```
*Source: `diagrams/mermaid/01-ingest/01l-network.mmd`*
...
```
*Source: `diagrams/mermaid/01-ingest/01m-deployment-boundaries.mmd`*
...
│   │   ├── src/
│   │   │   ├── api/             ← FastAPI (Fargate)
│   │   │   ├── stages/          ← Stage subclasses (patterns D6)
│   │   │   ├── adapters/        ← textract, docling, libreoffice, s3 (patterns A9)
│   │   │   ├── sandbox/         ← sandbox service image
│   │   │   └── workflow/        ← Step Functions definition (ASL, generated by CDK)
